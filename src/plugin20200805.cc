...
typedef struct Joint
{
	...
	int arr_flag = 0;
        // ***************행렬 선언*********************//
        double Theo_RL_PR[6] = {0.,0.,0.,0.,0.,0.}, Theo_LL_PR[6] = {0.,0.,0.,0.,0.,0.};
...
/*
void Inverse_Matrix(int n, double input_matrix[12][12], double output_matrix[12][12]){
    	//declare and initialize variables
	int i = 0, j = 0;
	float determinant = 0;
	// n means size of matrix
...
void Inverse_Matrix(int size, double input_matrix[12][24], double output_matrix[12][12]){
    // Declare variable
    // Array can be no larger than 10x10
    double Temp_matrix[2][24] = {{0.,},};
    double x = 0.;
	...
	{
		for (int b = 0; b < size*2; b++) //for (b = size; b < size*2; b++)
		{
			std::cout << "\t" << array[a][b];
		}
	...
	for (int a = 0; a < size; a++)
	{
		for (int b = size; b < 2 * size; b++) // 12X12 matrix이면 array의 b의 크기가 24이상이어야됨.
		{
			if (a == b - size)
				array[a][b] = 1;
			else
	...
	for (int a = 0; a < size; a++)
	{
				...
				if(array[i][0] != 0){
					...
					if(arr_flag == 1){
						for(int j =0; j< size*2; j++){
							Temp_matrix[0][j] = array[i][j];
							Temp_matrix[1][j] = array[0][j];
						}
						for(int j =0; j< size*2; j++){
							array[i][j] = Temp_matrix[1][j];
							array[0][j] = Temp_matrix[0][j]; 
						}
		...
		x = array[a][a];
		for (int b = a; b < 2 * size; b++)
		{
			array[a][b] = array[a][b] / x;			
		}
		for(int b =0; b<size*2; b++) std::cout << "array["<<a<<"]["<<b<<"] = " << array[a][b] << std::endl;
	...
	std::cout << "Gaussian elimination array:" << std::endl;
		for (int a = 0; a < size; a++)
		{
			for (int b = 0; b < size*2; b++) //for (b = size; b < size*2; b++)
			{
	...
	for (int a = 0; a < size; a++)
	{
		for (int b = 0; b < size*2; b++) //for (b = size; b < size*2; b++)
		{
			//output_matrix[a][b-size] = array[a][b];
			std::cout << "\t" << array[a][b];
		}
		std::cout << std::endl; // 엔터
	}
...
void BRP_RL_IK(double Ref_RL_RP[6], double Init_th[6], double IK_th[6]){  // (2015_07_29)
	...
	for (iter = 0; iter < 100; iter++) {
			...
			if (th[3] < 0){  // ¹«ž­°üÀýÀÌ (-)  ·Î Œö·ÅÇÏŽÂ °ÍÀ» ¹æÁö 
				th[3] = -th[3];
			}
...
void BRP_12DOF_IK(double Ref_RP[12], double Init_th[12], double IK_th[12]){  // 12DOF IK푸는식
	...
	for (int iter = 0; iter < 100; iter++) { //iter<100
			...
			for(int i = 0; i<6; i++){ PR[i] = RL_PR[i]; PR[i+6] = LL_PR[i];}
			...
			for(int i=0; i<12; i++){
				...
				if(i<6) New_Q4J_R[i] = New_Q4J[i];
				if(i>5) New_Q4J_L[i-6] = New_Q4J[i];
				...
				for(int j = 0; j<6;j++){PR[j] = RL_PR[j]; PR[j+6] = LL_PR[j];}
				for(int j = 0; j<12; j++) New_PR[j][i] = PR[j];
			...
			std::cout <<"eigen_J : "<< std::endl; // 엔터	
			std::cout <<""<< std::endl; // 엔터		
			std::cout << eigen_J << std::endl;

			eigen_J_Inv = eigen_J.inverse();

			std::cout <<"eigen_J_Inv : "<< std::endl; // 엔터	
			std::cout <<""<< std::endl; // 엔터		
			...
			if (th[9] < 0){  // ¹«ž­°üÀýÀÌ (-)  ·Î Œö·ÅÇÏŽÂ °ÍÀ» ¹æÁö
				th[9] = -th[9];
			}
...
void BRP_RL2PC_FK(double th[6], double PR[6]){  // (2015_07_29)
...
/*
    ...
    for(int i = 0; i < 6; i++) {
    std::cout << "Theo_RL_PR [" << i <<"] = " << Theo_RL_PR[i] << std::endl;
    }
    for(int i = 0; i < 6; i++) {
...
void gazebo::SUBO3_plugin::Pelvis_Center_Traj()
{
    ...
    BRP_RL_IK(Trans_Ref_RL_PR, RL_th, RL_th_IK);
    BRP_LL_IK(Trans_Ref_LL_PR, LL_th, LL_th_IK);

    for(int k=0;k<6;k++){ //IK랑 세트임
      RL_th[k] = RL_th_IK[k];
      LL_th[k] = LL_th_IK[k];
    }
